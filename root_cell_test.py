...
def generate_umap():
    ...
    summary2 = (f"Mean expression data: {str(global_mean_expression)}. "
                f"Expression proportion data: {str(global_expression_proportion)}. "
                f"Top genes details: {global_top_genes_df}. "
                f"markers: {marker_tree}. ")
    
    final_summary = marker_tree
...
if __name__ == "__main__":
    ...
    agent_content = """
    You are a bioinformatics researcher that can read marker and cell data.
    The data you will recieve contain several cells, markers, and their subsets.
    ...
    Goal: 
    1. Filter out all the root cells and its corresponding marker genes in the input data.
    2. The results should only contain root cells and its corresponding marker genes.
    3. Any cell exist in the subset should not be included.
    4. Do not include any additional words or explanations.
    """
    response = openai.chat.completions.create(
        model="gpt-4o",
        messages=[
