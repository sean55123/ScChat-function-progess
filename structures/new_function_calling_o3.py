...
class ScChat:
    ...
    def retrieve_stats(self):
        ...
        explanation = ("Please analyze the clustering statistics and classify each cluster based on the following data: "
                       "Top Genes:Mean Expression: Expression Proportion: , based on statistical data:"
                       "1. top_genes_df: 25 top genes expression within each clusters, with its p_val, p_val_adj, and logfoldchange; "
                       "2. mean_expression of the marker genes: specific marker genes mean expression within each cluster; "
                       "3. expression_proportion of the marker genes: every cluster each gene expression fraction within each cluster, and give back the mapping dictionary in the format like this "
                       "group_to_cell_type = {'0': 'Myeloid cells','1': 'T cells','2': 'Myeloid cells','3': 'Myeloid cells','4': 'T cells'} without further explanation or comment. "
    ...
    def start_chat2_web(self, user_input, conversation_history):
        ...
        self.display_flag = False
        scchat_context1 = f"""
        You are a helpful assistant tasked with Single-Cell RNA-seq Analysis.
        Here are the currently available set of functions in JSON format:
        function_descriptions: {function_descriptions}
        
        The function can be separated into categories:
        **Data process related** When 
        1. When encounter the prompt with filter out specific cells, call the process_cells since this function can gather required data for specific cells.
        2. When encounter the prompt with label the cluster, call the label_clusters for specific cells.
        
        **Visualization
        All the visualization related functions have the name display in their function name. Call them only when the prompt asks you to display plot or visualize results.
        
        **Statistic & calculation
        When encounter question that ask you to compare results, call the calculation related function.

        **If the function been called don't have an argument, leave a blank dictionary as argument**
        Please decides which function works the best in each secanrio, and DO NOT FORGET to respond with a greeting!
        """
        ...
        main_flag = False
        if response and output.tool_calls:
            main_flag = True
        if not main_flag:
        ...
        if output.tool_calls and main_flag:
...
if __name__ == "__main__":
    ...
    user_input = "Calculate the cell population chagnes in T cells before and after treatment?"
    print("Received message: ", user_input)
    final_response, conversation_history, display_flag = scchat.start_chat2_web(
        user_input=user_input,
        conversation_history=conversation_history
    ...
    user_input = "What's the possible reason that makes the population changes like this?"
    print("Received message: ", user_input)
    final_response, conversation_history, display_flag = scchat.start_chat2_web(
        user_input=user_input,
        conversation_history=conversation_history
