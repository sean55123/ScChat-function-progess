...
def generate_umap():
    ...
    return gene_dict, marker_tree, global_mean_expression, global_expression_proportion
...
if __name__ == "__main__":
    gene_dict, marker_tree, mean_expression, expression_proportion = generate_umap()  
    ...
    messages = [
        SystemMessage(content="""
            ...
            * 1. Gene list: top 25 cells in each cluster.
            * 2. Marker tree: marker genes that can help annotating cell type.           
            
            Use check the values of mean expression, expression proportion for each clusters to figure out the importance ranking order for the genes in the corresponding gene list.
            ...
            This means you should consider the markers in the marker tree to be more important. 
    ...
    #         expression[value] = []
    #         proportion[value] = []
    #     new_dict[value].append({key: gene_dict[key]})
    #     expression[value].append({key: mean_expression.loc[key].to_dict()})
    #     proportion[value].append({key: expression_proportion.loc[key].to_dict()})

    # annotation_result = {}
    # for cell in new_dict:
    #     spec_marker_tree = get_markers_by_effective_level_for_cell(cell, 2, cell_lookup)
    #     input_msg =(f"Top genes details: {new_dict[cell]}. "
    #                 f"Mean expression: {expression[cell]}. "
    #                 f"Expression proportion: {proportion[cell]}. "
    #                 f"Markers: {spec_marker_tree}. ")
    #     messages = [
    #         SystemMessage(content="""
    #             You are a bioinformatics researcher that can do the cell annotation.
    #             The following are the data and its decription that you will receive.
    #             * 1. Gene list: clusters and their corresponding top 25 cells.
    #             * 2. Mean expression: this is the average (mean) level of expression for each marker gene within each clusters.
    #             * 3. Expression proportion: this obtains the proportion (or fraction) of cells in each cluster that express each gene.
    #             * 4. Marker tree: marker genes that can help annotating cell type.           
    ...
    #             Provide your output in the following example format:
    #             {'0': 'Cell type','1': 'Cell type','2': 'Cell type','3': 'Cell type','4': 'Cell type', ...}.
                
    #             Strictly adhere to follwing rules:
    #             * 1. Adhere to the dictionary format and do not include any additional words or explanations.
    #             * 2. Annotated cell's name should only come from marker tree
    #             * 3. The cluster number in the result dictionary must be arranged with raising power.
    #             """),
    #         HumanMessage(content=input_msg)
    #     ]

    #     results = model.invoke(messages)
    #     result_str = results.content.strip()
    #     result_dict = ast.literal_eval(result_str)
    #     annotation_result.update(result_dict)
    # annotation_result = {k: annotation_result[k] for k in sorted(annotation_result, key=lambda x: int(x))}
    ...
    # expression = {}
    # proportion = {}
    # for key, value in annotation_result.items():
    #     if value not in new_dict:
    #         new_dict[value] = []
    #         expression[value] = []
    #         proportion[value] = []
