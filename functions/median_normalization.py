...
normalized_by_depth = adata.X.toarray()
n_genes = normalized_by_depth.shape[1]
gene_medians = np.zeros(n_genes)
for j in range(n_genes):
    nonzero_vals = normalized_by_depth[:, j][normalized_by_depth[:, j] > 0]
    gene_medians[j] = np.median(nonzero_vals) if nonzero_vals.size > 0 else 1
custom_normalized = normalized_by_depth / gene_medians[None, :]
adata.X = custom_normalized
...
sc.tl.rank_genes_groups(adata, groupby='leiden', method='wilcoxon', use_raw=False, n_genes=30)
